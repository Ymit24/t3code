...
function useActiveThread(threadId: ThreadId): Thread | undefined {
  ...
  const localDraftError = chatViewStore((store) => store.localDraftError);
...
export const createChatViewStore = (threadId: ThreadId) => {
  return createStore<ChatViewStore>((set, get) => ({
...
export function useChatViewStore() {
  const context = useContext(ChatViewStoreContext);
  if (!context) {
