...
export default function ChatExpandedImageViewer() {
  ...
  const openExpandedImage = useChatViewStore((store) => store.openExpandImage);
  const closeExpandedImage = useChatViewStore((store) => store.closeExpandedImage);

  const navigateExpandedImage = useCallback(
    (direction: -1 | 1) => {
      const existing = expandedImage;
      ...
      openExpandedImage({ ...existing, index: nextIndex });
    },
    [openExpandedImage, expandedImage],
  );
